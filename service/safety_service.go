...
func (s *safetyServiceImpl) CheckRateLimit(ctx context.Context, userID int64) (*RateLimitResult, error) {
	...
	if !allowed {
		...
		return &RateLimitResult{
			Allowed: false,
			Reason:  "rate_limited",
		}, errors.NewCode(errors.Validation, msg)
	}
...
func (s *safetyServiceImpl) DetectPII(ctx context.Context, content string) (*SafetyResult, error) {
	...
	if piiRegex.MatchString(content) {
		return &SafetyResult{Allowed: false, Reason: "pii_detected"}, errors.NewCode(errors.Validation, "内容包含敏感信息")
	}
	...
	for _, kw := range kws {
		...
		if strings.Contains(lower, strings.ToLower(kw)) {
			return &SafetyResult{
				Allowed: false,
				Reason:  "命中敏感词",
			}, errors.NewCode(errors.Validation, "内容命中敏感词")
		}
	}
	return &SafetyResult{Allowed: true}, nil
}
