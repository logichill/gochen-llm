...
func (s *promptServiceImpl) RenderPrompt(ctx context.Context, tmpl *entity.PromptTemplate, vars map[string]any) (string, error) {
	if tmpl == nil {
		return "", errorx.NewError(errorx.InvalidInput, "模板不能为空")
	}
	t, err := template.New("prompt").Parse(tmpl.Content)
	if err != nil {
		return "", errorx.WrapError(err, errorx.Internal, "解析提示词模板失败")
	}
	...
	if err := t.Execute(&buf, vars); err != nil {
		return "", errorx.WrapError(err, errorx.Internal, "渲染提示词模板失败")
	}
...
func (s *promptServiceImpl) SavePrompt(ctx context.Context, tmpl *entity.PromptTemplate) error {
	if tmpl == nil {
		return errorx.NewError(errorx.InvalidInput, "提示词模板不能为空")
	}
	if tmpl.Scope == "" {
...
func (s *promptServiceImpl) CreateVersion(ctx context.Context, templateID int64, changeLog string) (*entity.PromptVersion, error) {
	if templateID <= 0 {
		return nil, errorx.NewError(errorx.InvalidInput, "templateID 无效")
	}
	...
	if tmpl == nil {
		return nil, errorx.NewError(errorx.NotFound, "提示词模板不存在")
	}
...
func (s *promptServiceImpl) RollbackVersion(ctx context.Context, templateID int64, version int) error {
	if templateID <= 0 || version <= 0 {
		return errorx.NewError(errorx.InvalidInput, "templateID 或 version 无效")
	}
	...
	if target == nil {
		return errorx.NewError(errorx.NotFound, "指定版本不存在")
	}
	...
	if tmpl == nil {
		return errorx.NewError(errorx.NotFound, "提示词模板不存在")
	}
...
func (s *promptServiceImpl) ImportPrompts(ctx context.Context, data []byte) error {
	...
	if err := json.Unmarshal(data, &list); err != nil {
		return errorx.WrapError(err, errorx.InvalidInput, "解析导入数据失败")
	}
...
func (s *promptServiceImpl) StartABTest(ctx context.Context, test *entity.ABTest) error {
	...
	if test.TemplateAID <= 0 || test.TemplateBID <= 0 {
		return errorx.NewError(errorx.Validation, "A/B 测试模板 ID 无效")
	}
...
func (s *promptServiceImpl) AssignABVariant(ctx context.Context, testID int64, userID int64) (*entity.PromptTemplate, string, error) {
	if testID <= 0 {
		return nil, "", errorx.NewError(errorx.InvalidInput, "ab_test_id 无效")
	}
	test, err := s.repo.GetABTest(ctx, testID)
	if err != nil {
		return nil, "", err
	...
	if test == nil || test.Status != "running" {
		return nil, "", errorx.NewError(errorx.NotFound, "A/B 测试不可用")
	}
	...
	if tmpl == nil {
		return nil, "", errorx.NewError(errorx.NotFound, "A/B 变体模板不存在")
	}
