...
func (r *providerConfigRepoImpl) ListAll(ctx context.Context) ([]*entity.ProviderConfig, error) {
	...
	if err := model.Find(ctx, &cfgs,
		...
		return nil, errorx.WrapDbError(ctx, err, "查询 LLM provider 配置失败")
	}
	return cfgs, nil
}
...
func (r *providerConfigRepoImpl) ReplaceAll(ctx context.Context, configs []*entity.ProviderConfig) error {
	...
	if err != nil {
		return errorx.WrapDbError(ctx, err, "开启 LLM provider 配置事务失败")
	}
	committed := false
	defer func() {
		if !committed {
	...
	if len(configs) > 0 {
		if err := model.Create(ctx, anyPtrSlice(configs)...); err != nil {
			return errorx.WrapDbError(ctx, err, "批量保存 LLM provider 配置失败")
		}
	}

	if err := session.Commit(); err != nil {
		return errorx.WrapDbError(ctx, err, "提交 LLM provider 配置事务失败")
	}
	committed = true
	return nil
}
...
func (r *providerConfigRepoImpl) UpdatePricing(ctx context.Context, updates []entity.ProviderPricing) error {
	...
	session, err := r.orm.Begin(ctx)
	if err != nil {
		return errorx.WrapDbError(ctx, err, "开启更新 LLM 单价事务失败")
	}
	committed := false
	defer func() {
		if !committed {
	...
	for _, up := range updates {
		if up.ID <= 0 {
			return errorx.NewError(errorx.InvalidInput, "pricing id 无效")
		}
		if up.InputPricePer1k < 0 || up.OutputPricePer1k < 0 {
			return errorx.NewError(errorx.Validation, "单价不能为负数")
		}
		...
		if err := model.UpdateValues(ctx, updateValues, orm.WithWhere("id = ?", up.ID)); err != nil {
			return errorx.WrapDbError(ctx, err, "更新 LLM 单价失败")
		}
	}

	if err := session.Commit(); err != nil {
		return errorx.WrapDbError(ctx, err, "提交更新 LLM 单价事务失败")
	}
	committed = true
	return nil
}
