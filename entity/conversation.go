...
type Conversation struct {
	...
	UserID   int64  `gorm:"not null;index:idx_user_id"` // 归属用户 ID
	ParentID *int64 `gorm:"index:idx_parent_id"`        // 父会话 ID（用于会话分支）
	...
	Status string `gorm:"size:20;not null;default:'active';index:idx_status"` // 会话状态，如 active/archived

	// PromptTemplateID 关联的提示词模板（如故事世界）
	PromptTemplateID *int64 `gorm:"index:idx_prompt_template_id"` // 关联的提示词模板 ID
...
type Message struct {
	...
	ConversationID int64     `gorm:"not null;index:idx_conversation_id"` // 所属会话 ID
