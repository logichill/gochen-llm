...
type SafetyPolicy struct {
	...
	GlobalSystemPrompt string `gorm:"type:text"`

	// 屏蔽类别（JSON 数组），预留给不同 Provider 的安全设置适配
	BlockedCategoriesJSON string `gorm:"type:text"`

	// 屏蔽关键词（JSON 数组），用于输入/输出的简单文本过滤
	BlockedKeywordsJSON string `gorm:"type:text"`

	// 生成内容的最大长度（字符数，0 表示不限制）
	MaxContentLength int `gorm:"not null;default:0"`

	// 日志级别：none / summary / full_violation 等（首版仅记录占位）
	LogLevel string `gorm:"size:20;not null;default:'none'"`
